...
26. **Polish the steak / seafood / dessert placeholder icons** — Added in this session (`icon-steak.svg`, `icon-seafood.svg`, `icon-cocktail.svg`, `icon-dessert.svg`). The cocktail icon reads well; steak, seafood, and the cupcake (dessert) are usable but the linework could be cleaner/more recognizable at a glance. Worth a dedicated artwork pass with Opus to redraw those three in the existing style (cream fill, dark-green stroke, gold/sage accents).
